...
fn fragment(in: VertexOutput) -> @location(0) vec4<f32> {
	...
	for (var i = 0u; i < 25; i += 1u) {
		...
		let point1: vec2<f32> = in.world_position.xy - left;
		let point2: vec2<f32> = in.world_position.xy - right;
		...
		var p: vec2<f32> = pos - in.world_position.xy + t * v;
	...
	rgb = vec4(rgb.xyz + (hash33(vec3<f32>(in.world_position.xy, globals.time * 256.)) / 512.), rgb.w);
